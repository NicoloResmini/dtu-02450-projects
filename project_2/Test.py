...
outer_fold_i = np.reshape(range(1, K1 + 1), (2, 1))
optimal_ANN = np.reshape(optimal_ANN, (2, 1))
Etest_ANN = np.reshape(Etest_ANN, (2, 1))

# Convert numpy arrays to lists before applying the format() function
dff = pd.DataFrame(list(zip(outer_fold_i.tolist(), optimal_ANN.tolist(), Etest_ANN.tolist(), opt_lambda.tolist(), Error_test_rlr.tolist(), Etest_nofeatures.tolist())), columns=columns)

 #%%
for col in dff.columns:
    print(col, type(dff[col].iloc[0]))
