...
df = pd.DataFrame(list(zip(range(1, K1 + 1), optimal_ANN.tolist(), Etest_ANN.tolist(), opt_lambda.tolist(), Error_test_rlr.tolist(), Etest_nofeatures.tolist())), columns=columns)

# Allinea i valori al centro delle celle
df_styled = df.style.set_properties(**{'text-align': 'center'}).format("{:.2f}")
